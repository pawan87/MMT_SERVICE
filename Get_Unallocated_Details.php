	<?php
	/**
		* Webservice for, Fetch all query list information whose status is N(Painding) or P(Inprogress) or cs(done).
	...
	{		
		...
		$category = $_REQUEST['category'];
			...
			if(!empty($queryID)){
				...
				$dataQueryInfo = "SELECT * FROM Query_Details_T WHERE QUERYID >'$queryID' AND TRAVEL_CATEGORY='$category' AND STATUS='N'";
				$dataResultSet = mysql_query($dataQueryInfo);
			}
			else{
				
				$dataQueryInfo = "(SELECT * FROM Query_Details_T WHERE TRAVEL_CATEGORY='domestic' AND STATUS='N' ORDER BY TRAVEL_CATEGORY) UNION (SELECT * FROM Query_Details_T WHERE TRAVEL_CATEGORY='international' AND STATUS='N' ORDER BY TRAVEL_CATEGORY)";
			    $dataResultSet = mysql_query($dataQueryInfo);
			}
