	<?php
	/**
		* Webservice for, Fetch all query list information whose status is  CS(done).
	
	*/
	include_once('DBConnect.php'); //Database Connection
	function getCompleteQueryDetails()
	{		
		$userID = $_REQUEST['userID']; //Get Request From Device
           ...
           if( $userID == "" ){
				...
				$queryList = array();
				$dataQueryInfo = "SELECT * FROM Query_Details_T WHERE USERID='$userID' AND STATUS='CS'";
